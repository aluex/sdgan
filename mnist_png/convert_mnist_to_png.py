...
def write_dataset(labels, data, size, rows, cols, output_dir):
    ...
    for (i, label) in enumerate(labels):
        output_filename = output_dir + '/' + str(label) + '_' + str(i) + '_0.png'  # path.join(output_dirs[label], str(i) + ".png")
        ...
        imt = im.resize((64, 64), Image.ANTIALIAS).convert('RGB')
        imt.save(output_filename)
